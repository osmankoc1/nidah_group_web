...
export async function getLocaleBlogPostData(locale: TranslationLocale, slug: string) {
  ...
  // Fetch all available locale slugs for the language switcher + hreflang
          ...
          {postTags.length > 0 && (
            <div className="flex flex-wrap gap-2 mt-10 pt-8 border-t" dir="ltr">
              ...
              {postTags.map(tag => (
                <span
                  key={tag.slug}
                  className="text-xs bg-nidah-light border border-gray-200 text-nidah-dark rounded-full px-3 py-1"
                >
                  {tag.name}
                </span>
              ))}
